#!/bin/bash
# =============================================================================
# 清理脚本：杀掉 examples/auth 示例残留的 provider / consumer 进程
#
# 使用方法:
#   chmod +x cleanup.sh
...
#
# 注意:
#   verify_auth.sh 自身已在退出 trap 中通过 Polaris OpenAPI 删除本服务下所有
#   BlockAllowListRule，因此 cleanup.sh 不重复调用 OpenAPI 清规则。如怀疑残留，
#   重跑一次 ./verify_auth.sh（首步会再清理一遍），或在控制台手工删除。
# =============================================================================
...
# 查找 auth provider 进程：匹配 .build/auth_provider 或 examples/auth/provider/.../provider
while IFS= read -r line; do
    pid=$(echo "$line" | awk '{print $2}')
    PROVIDER_PIDS+=("$pid")
    PROVIDER_DESCS+=("$line")
done < <(ps -ef \
    | grep -E 'examples/auth/.*/auth_provider|\.build/auth_provider|examples/auth/provider/(\.build/)?provider\b' \
    | grep -v grep)
...
while IFS= read -r line; do
    ...
    | grep -E 'examples/auth/.*/auth_consumer|\.build/auth_consumer|examples/auth/consumer/(\.build/)?consumer\b' \
    | grep -v grep)
...
if [[ ${#PROVIDER_PIDS[@]} -eq 0 ]] && [[ ${#CONSUMER_PIDS[@]} -eq 0 ]]; then
    echo -e "${GREEN}未发现残留的 auth provider/consumer 进程，无需清理。${NC}"
    cleanup_dirs
    echo ""
    exit 0
fi
...
if [[ ${#PROVIDER_PIDS[@]} -gt 0 ]]; then
    echo -e "${YELLOW}发现 ${#PROVIDER_PIDS[@]} 个残留 Auth Provider 进程:${NC}"
    echo ""
...
if [[ ${#CONSUMER_PIDS[@]} -gt 0 ]]; then
    echo -e "${YELLOW}发现 ${#CONSUMER_PIDS[@]} 个残留 Auth Consumer 进程:${NC}"
    echo ""
