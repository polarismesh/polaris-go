...
```
   ...
   │     .build/fault_detect_consumer_run/
   ...
   │     .build/fd_instance_consumer_run/
   ...
   │     .build/fd_udp_consumer_run/
   │     用例五 UDP 探测（abort 型，SERVICE 级，探测端口 28101）
   └── 用例六 协议/方法维度 + Headers 验证（复用 18095/18096 consumer，不新增进程）
         .build/fault_detect_consumer_run/（段 A/C）+ .build/fd_method_consumer_run/（段 B）
```
...
下表是"每个用例到底用哪条熔断规则 + 哪条探测规则"的权威对照。**用例一~五各自创建一对独立的熔断规则 + 探测规则；用例六不创建熔断规则、复用用例一/二的，探测规则只新建 1 条 `-nonmatch`，其余复用。**

| 用例 | consumer | 熔断规则（控制 OPEN/CLOSE） | 探测规则（控制怎么探） | 探测规则来源 |
|------|----------|------------------------------|------------------------|--------------|
| 一 服务级(SERVICE) | fault_detect_consumer (18095) | `cb-fd-SERVICE-CircuitBreakerFaultDetectCaller` | `fd-fd-SERVICE-CircuitBreakerFaultDetectCaller`（HTTP, method=`*`） | 本用例新建 |
| 二 接口级(METHOD) | fd_method_consumer (18096) | `cb-fd-METHOD-CircuitBreakerFDMethodCaller` | `fd-fd-METHOD-CircuitBreakerFDMethodCaller`（HTTP, method=`/echo`） | 本用例新建 |
| 三 实例级(INSTANCE) | fd_instance_consumer (18097) | `cb-fd-INSTANCE-CircuitBreakerFDInstanceCaller` | `fd-fd-INSTANCE-CircuitBreakerFDInstanceCaller`（HTTP, method=`*`） | 本用例新建 |
| 四 TCP 探测 | fd_tcp_consumer (18098) | `cb-fd-TCP-CircuitBreakerFDTcpCaller` | `fd-fd-TCP-CircuitBreakerFDTcpCaller`（TCP, port=28091） | 本用例新建 |
| 五 UDP 探测 | fd_udp_consumer (18099) | `cb-fd-UDP-CircuitBreakerFDUdpCaller` | `fd-fd-UDP-CircuitBreakerFDUdpCaller`（UDP, port=28101） | 本用例新建 |
| 六-段A | fault_detect_consumer (18095)<br>**复用用例一** | `cb-fd-SERVICE-CircuitBreakerFaultDetectCaller`<br>**复用用例一** | `fd-fd-SERVICE-CircuitBreakerFaultDetectCaller`（method=`*`）<br>**复用用例一** | 复用用例一，不新建 |
| 六-段B | fd_method_consumer (18096)<br>**复用用例二** | `cb-fd-METHOD-CircuitBreakerFDMethodCaller`<br>**复用用例二** | 选中：`fd-fd-METHOD-CircuitBreakerFDMethodCaller`（method=`/echo`，**复用用例二**）<br>拒绝：`fd-fd-METHOD-CircuitBreakerFDMethodCaller-nonmatch`（method=`/api/protocol/http`，**本段新建**） | /echo 复用用例二；`-nonmatch` 本段新建 |
| 六-段C | fault_detect_consumer (18095)<br>**复用用例一** | `cb-fd-SERVICE-CircuitBreakerFaultDetectCaller`<br>**复用用例一** | `fd-fd-SERVICE-CircuitBreakerFaultDetectCaller`（PUT 加 `X-Health-Probe` header）<br>**复用用例一并 PUT 更新** | 复用用例一，PUT 改 header |

**规则总数**：6 条熔断规则（用例一~五各 1 条；用例六全部复用）+ 6 条探测规则（用例一~五各 1 条 + 用例六段 B 的 `-nonmatch` 1 条；段 A/C 复用用例一的）。

> **熔断规则 vs 探测规则的职责区分**（理解对应关系的关键）：
> - **熔断规则**（`cb-fd-*`）：定义"什么时候熔断"——CONSECUTIVE_ERROR 阈值、sleepWindow、错误条件、状态机 OPEN/HALF_OPEN/CLOSE。同时它的 `faultDetectConfig.enable=true` 是**探测启动门控**。
> - **探测规则**（`fd-fd-*`）：定义"怎么探"——探测协议（HTTP/TCP/UDP）、探测路径/端口、interval、method 过滤。
> - 两者是**独立的规则类型**，通过 `target_service`（被调服务）关联：熔断器进入 OPEN 后，SDK 按探测规则周期性探活被调实例，探活成功推动 HALF_OPEN→CLOSE。
...
> **本节两个用例各用什么规则**：
> - **用例一（SERVICE）**：熔断规则 `cb-fd-SERVICE-CircuitBreakerFaultDetectCaller` + 探测规则 `fd-fd-SERVICE-CircuitBreakerFaultDetectCaller`（method=`*`）
> - **用例二（METHOD）**：熔断规则 `cb-fd-METHOD-CircuitBreakerFDMethodCaller` + 探测规则 `fd-fd-METHOD-CircuitBreakerFDMethodCaller`（method=`/echo`）
...
### 规则
- 熔断规则：
  - SERVICE 级 `cb-fd-SERVICE-CircuitBreakerFaultDetectCaller`，`level=SERVICE`
  - METHOD 级 `cb-fd-METHOD-CircuitBreakerFDMethodCaller`，`level=METHOD`
- METHOD 级额外限定 `destination.method.value=/echo`，`BlockConfig.api.path=/echo`
- 公共：`CONSECUTIVE_ERROR=5`，`sleepWindow=6s`，`faultDetectConfig.enable=true`，**仅 CONSECUTIVE_ERROR 不叠加 ERROR_RATE**
- 探测规则：
  - SERVICE 级 `fd-fd-SERVICE-CircuitBreakerFaultDetectCaller`（method=`*`，通配）
  - METHOD 级 `fd-fd-METHOD-CircuitBreakerFDMethodCaller`（method=`/echo`）
  - 公共：`protocol=HTTP`(1)，`GET /echo`，`interval=2s`，`timeout=1000ms`，`port=0`
...
> **本用例用什么规则**：熔断规则 `cb-fd-INSTANCE-CircuitBreakerFDInstanceCaller` + 探测规则 `fd-fd-INSTANCE-CircuitBreakerFDInstanceCaller`（method=`*`）
...
### 规则
- 熔断规则：`cb-fd-INSTANCE-CircuitBreakerFDInstanceCaller`
- `level=INSTANCE`，`CONSECUTIVE_ERROR=5`，`sleepWindow=6s`，`faultDetectConfig.enable=true`
- 探测规则：`fd-fd-INSTANCE-CircuitBreakerFDInstanceCaller`，`protocol=HTTP`(1)，`GET /echo`，`interval=2s`，`port=0`
...
> **本节两个用例各用什么规则**：
> - **用例四（TCP）**：熔断规则 `cb-fd-TCP-CircuitBreakerFDTcpCaller`（SERVICE 级）+ 探测规则 `fd-fd-TCP-CircuitBreakerFDTcpCaller`（TCP, port=28091）
> - **用例五（UDP）**：熔断规则 `cb-fd-UDP-CircuitBreakerFDUdpCaller`（SERVICE 级）+ 探测规则 `fd-fd-UDP-CircuitBreakerFDUdpCaller`（UDP, port=28101）
...
### 规则
- 熔断规则：
  - TCP 用例 `cb-fd-TCP-CircuitBreakerFDTcpCaller`，`level=SERVICE`
  - UDP 用例 `cb-fd-UDP-CircuitBreakerFDUdpCaller`，`level=SERVICE`
- 公共：`CONSECUTIVE_ERROR=5`，`sleepWindow=6s`，`faultDetectConfig.enable=true`
- TCP 探测规则：`fd-fd-TCP-CircuitBreakerFDTcpCaller`，`protocol=TCP`(2)，`port=28091`，`tcp_config={send:"ping", receive:["tcp-ok"]}`，`interval=2s`，`timeout=1000ms`
- UDP 探测规则：`fd-fd-UDP-CircuitBreakerFDUdpCaller`，`protocol=UDP`(3)，`port=28101`，`udp_config={send:"ping", receive:["udp-ok"]}`，`interval=2s`，`timeout=1000ms`
...
> **本用例用什么规则**（全部复用前序用例，仅段 B 新建 1 条探测规则）：
> - **段 A**：复用用例一的熔断规则 `cb-fd-SERVICE-CircuitBreakerFaultDetectCaller` + 探测规则 `fd-fd-SERVICE-CircuitBreakerFaultDetectCaller`（method=`*`）
> - **段 B**：复用用例二的熔断规则 `cb-fd-METHOD-CircuitBreakerFDMethodCaller`；探测规则 `/echo` 复用用例二的 `fd-fd-METHOD-CircuitBreakerFDMethodCaller`（应选中），**仅新建** `fd-fd-METHOD-CircuitBreakerFDMethodCaller-nonmatch`（应拒绝）
> - **段 C**：复用用例一的熔断规则 + 探测规则 `fd-fd-SERVICE-CircuitBreakerFaultDetectCaller`（PUT 加 `X-Health-Probe` header）
...
### Caller 写法
- 段 A/C 复用 SERVICE consumer（端口 18095），复用熔断规则 `cb-fd-SERVICE-CircuitBreakerFaultDetectCaller`
- 段 B 复用 METHOD consumer（端口 18096），复用熔断规则 `cb-fd-METHOD-CircuitBreakerFDMethodCaller`

### 规则
- 熔断规则：全部复用已有，不创建新熔断规则
- 段 A 探测规则：复用已有 `fd-fd-SERVICE-CircuitBreakerFaultDetectCaller`（method=*，不创建新规则）
- 段 B 探测规则：`/echo` 匹配复用用例二已有的 `fd-fd-METHOD-CircuitBreakerFDMethodCaller`（method=/echo，应选中，不新建）；仅新建 `fd-fd-METHOD-CircuitBreakerFDMethodCaller-nonmatch`（method=/api/protocol/http，应拒绝）
- 段 C 探测规则：PUT 更新已有 `fd-fd-SERVICE-CircuitBreakerFaultDetectCaller`，加 `http_config.headers=[{key:"X-Health-Probe", value:"true"}]`

### 验证步骤

#### 段 A：SERVICE 级 method 过滤（复用已有规则，不创建新规则）

| 步骤 | 动作 | 判定指标 |
|------|------|---------|
| [A] | 验证已有 SERVICE 探测规则 `fd-fd-SERVICE-CircuitBreakerFaultDetectCaller`（method=`*` 通配）已被 schedule task 选中 | `schedule task` 含该规则名（SERVICE 级接受通配 method）；非通配拒绝由段 B.2 等价覆盖 |

> **A 段不创建新规则**：SDK 的 `selectFaultDetectRules` 对同 protocol 只取第一条匹配的探测规则
> （`checker.go` 中 `if _, ok := matchRule[protocol]; !ok`），SERVICE 用例的
> `fd-fd-SERVICE-CircuitBreakerFaultDetectCaller`（method=`*`）已占用 HTTP 协议 slot，
> 新规则不会被选中。该规则 method=`*` 本身就是通配，它被 schedule task 选中
> 即证明了"SERVICE 级接受通配 method"。

#### 段 B：METHOD 级 method 过滤 + 完整闭环

| 步骤 | 动作 | 判定指标 |
|------|------|---------|
| [B.1] | 复用用例二的 `fd-fd-METHOD-CircuitBreakerFDMethodCaller`（method=`/echo`，应被选中）+ 新建 `fd-fd-METHOD-CircuitBreakerFDMethodCaller-nonmatch`（method=`/api/protocol/http`，应被拒绝） | `schedule task` 含 `path=/echo`（复用规则被选中），`-nonmatch` 不出现 schedule task |
...
| 步骤 | 动作 | 判定指标 |
|------|------|---------|
| [C.1] | PUT 更新已有探测规则 `fd-fd-SERVICE-CircuitBreakerFaultDetectCaller`，添加 `http_config.headers=[{key:"X-Health-Probe", value:"true"}]` | `schedule task` 出现 |
...
### 通过条件
- **段 A**：`has_a1=yes`（拒绝非通配，由段 B.2 METHOD 级 nonmatch 等价覆盖）且 `has_a2=yes`（接受通配，已有 SERVICE 规则 method=* 被 schedule task 选中）
...
### 验证原理
- **段 A SERVICE 级 method 过滤**：`selectFaultDetectRules`（`checker.go`）对 SERVICE 级资源只接受
  `targetService.method.value` 为空或 `*` 的探测规则（`match.IsMatchAll`），非通配值会被过滤。
  SERVICE 用例已创建的 `fd-fd-SERVICE-*` 规则 method=`*` 本身就是通配，验证该规则已被 schedule task
  选中即可证明"SERVICE 级接受通配 method"。非通配拒绝验证由段 B.2 METHOD 级 nonmatch 等价覆盖——
  METHOD 级同样走 `matchMethodWithAPI` → `IsMatchAll` 路径，METHOD 级拒绝 → SERVICE 级同理。
  **不创建新规则**的原因：`selectFaultDetectRules` 对同 protocol 只取第一条匹配规则（`if _, ok := matchRule[protocol]; !ok`），
  SERVICE 用例已有规则已占 HTTP 协议 slot，新规则不会被选中。
- **段 B METHOD 级 method 过滤**：METHOD 级资源调用 `matchMethod` 按 `resource.Path` 精确匹配 `targetService.method`——`fd-fd-METHOD-*-nonmatch`（method=`/api/protocol/http`）不与 `/echo` 匹配，因此不被选中；复用的 `fd-fd-METHOD-CircuitBreakerFDMethodCaller`（method=`/echo`，用例二创建）匹配，被选中为探测规则。`/echo` 探测规则无需在段 B 重建——用例二已有的同 method 规则完全等价，避免在同一服务下堆积冗余探测规则。
...
| 用例 | SDK 日志目录 |
|------|-------------|
| 用例一 服务级(SERVICE) | `.build/fault_detect_consumer_run/polaris/log/circuitbreaker/polaris-circuitbreaker.log` |
| 用例二 接口级(METHOD) | `.build/fd_method_consumer_run/polaris/log/circuitbreaker/polaris-circuitbreaker.log` |
| 用例三 实例级(INSTANCE) | `.build/fd_instance_consumer_run/polaris/log/circuitbreaker/polaris-circuitbreaker.log` |
| 用例四 TCP 探测 | `.build/fd_tcp_consumer_run/polaris/log/circuitbreaker/polaris-circuitbreaker.log` |
| 用例五 UDP 探测 | `.build/fd_udp_consumer_run/polaris/log/circuitbreaker/polaris-circuitbreaker.log` |
| 用例六 协议/方法 | 段 A/C 复用 `.build/fault_detect_consumer_run/`；段 B 复用 `.build/fd_method_consumer_run/` |
| 应用日志 | `.logs/fd_*.log`（consumer stdout，**不含 SDK 内部日志**） |
| 探测失败日志 | `.build/<consumer_name>_run/polaris/log/detect/polaris-detect.log`（仅网络异常时落盘） |
| 事件日志 | `.build/<consumer_name>_run/polaris/log/event/polaris-event.log`（熔断事件上报） |
...
| `selectFaultDetectRules` method 过滤 | 用例六 段 A（SERVICE 接受通配，复用已有规则验证）/ 段 B（METHOD 按 path 精确匹配） |
