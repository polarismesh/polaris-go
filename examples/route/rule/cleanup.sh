#!/bin/bash
# =============================================================================
# 清理脚本：杀掉 rule route 示例残留的 provider / consumer 进程
#
# 使用方法:
#   chmod +x cleanup.sh
...
echo ""
echo -e "${CYAN}========================================${NC}"
echo -e "${CYAN}  Rule Route 示例进程清理工具${NC}"
echo -e "${CYAN}========================================${NC}"
echo ""
...
# 查找 provider 进程（匹配 rule route 的 provider）
while IFS= read -r line; do
    pid=$(echo "$line" | awk '{print $2}')
    PROVIDER_PIDS+=("$pid")
    PROVIDER_DESCS+=("$line")
done < <(ps -ef | grep -E '\.build/(provider_[a-z]+/)?provider\b' | grep -v grep | grep -v fileproviderd | grep -i 'RouteEcho\|rule')

# 查找 consumer 进程（匹配 rule route 的 consumer）
while IFS= read -r line; do
    pid=$(echo "$line" | awk '{print $2}')
    CONSUMER_PIDS+=("$pid")
    CONSUMER_DESCS+=("$line")
done < <(ps -ef | grep -E '\.build/(consumer_run/)?consumer\b' | grep -v grep | grep -i 'RouteEcho\|rule')
...
if [[ ${#PROVIDER_PIDS[@]} -eq 0 ]] && [[ ${#CONSUMER_PIDS[@]} -eq 0 ]]; then
    echo -e "${GREEN}未发现残留的 rule route provider/consumer 进程，无需清理。${NC}"
    cleanup_dirs
    echo ""
    exit 0
fi
